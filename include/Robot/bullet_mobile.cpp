#include "bullet_mobile.h"


void print_vec(Mob_JVec vec,string str){
	cout<<str<<":";
...
Bullet_Mobile::Bullet_Mobile(class b3RobotSimulatorClientAPI* sim, int robotId){
	this->sim = sim;
...
Mob_JVec saturate(Mob_JVec val, Mob_JVec max_val){
	Mob_JVec retVal = Map<Mob_JVec>(val.data(), val.rows(), val.cols());
	for (int i = 0; i<val.size();i++){
		retVal[i] = min(max(val[i],-max_val[i]),max_val[i]);
	}
	return retVal;
}
void Bullet_Mobile::set_torque(Mob_JVec  torques ,Mob_JVec  max_torques){
	b3RobotSimulatorJointMotorArgs controlArgs(CONTROL_MODE_TORQUE);
	Mob_JVec saturated_torques = saturate(torques,max_torques);
...
Mob_JVec Bullet_Mobile::get_q(){
	Mob_JVec q(this->actuated_joint_num);
...
Mob_JVec Bullet_Mobile::get_qdot(){
	Mob_JVec qdot(this->actuated_joint_num);
	b3JointSensorState jointStates;
	int numJoints = sim->getNumJoints(this->robotId);
...
SE3 Bullet_Mobile::get_eef_pose(){
...
void Bullet_Mobile::reset_q(Mob_JVec q){
	for (int i = 0; i<q.size();i++){
		sim->resetJointState(this->robotId,this->actuated_joint_id.at(i),q[i]);
	}
}
void Bullet_Mobile::apply_ext_FT(Mob_JVec FT){
	btVector3 force ;
	btVector3 torque ;
	torque[0] = -FT(0);
	torque[1] = -FT(1);
