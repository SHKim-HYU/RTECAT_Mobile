...
namespace lr {
	...
	SE3 FKinSpace(const SE3& M, const Mob_ScrewList& Slist, const Mob_JVec& thetaList) {
		SE3 T = M;
		for (int i = (MOBILE_DOF_NUM - 1); i > -1; i--) {
	...
	Mob_Jacobian JacobianSpace(const Mob_ScrewList& Slist, const Mob_JVec& thetaList) {
		Mob_Jacobian Js = Slist;	
		SE3 T = SE3::Identity();
		for (int i = 1; i < MOBILE_DOF_NUM; i++) {
	...
	Mob_Jacobian JacobianBody(const Mob_ScrewList& Blist, const Mob_JVec& thetaList) {
		Mob_Jacobian Jb = Blist;
		SE3 T = SE3::Identity();
		for (int i = MOBILE_DOF_NUM -2; i >= 0; i--) {
	...
	bool IKinBody(const Mob_ScrewList& Blist, const SE3& M, const SE3& T,
		...
		while (err && i < maxiterations) {
			...
			Jb = Eigen::Map<Eigen::MatrixXd>(Jb_.data(),6,MOBILE_DOF_NUM);
	...
	bool IKinSpace(const Mob_ScrewList& Slist, const SE3& M, const SE3& T,
		...
		while (err && i < maxiterations) {
			...
			Js = Eigen::Map<Eigen::MatrixXd>(Js_.data(),6,MOBILE_DOF_NUM);
	...
	Mob_JVec InverseDynamics(const Mob_JVec& thetalist, const Mob_JVec& dthetalist, const Mob_JVec& ddthetalist,
									...
									const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist) {
	    // the size of the lists
		int n = MOBILE_DOF_NUM;

		SE3 Mi = SE3::Identity();
		Mob_Matrix6xn Ai = Mob_Matrix6xn::Zero();
		vector<Matrix6d> AdTi;
		for (int i = 0; i < n+1; i++) {
			AdTi.push_back(Matrix6d::Zero());
		}
		Mob_Matrix6xn_1 Vi = Mob_Matrix6xn_1::Zero();    // velocity
		Mob_Matrix6xn_1 Vdi = Mob_Matrix6xn_1::Zero();   // acceleration
		...
		Mob_JVec taulist = Mob_JVec::Zero();
	...
	Mob_JVec InverseDynamics(const Mob_JVec& thetalist, const Mob_JVec& dthetalist, const Mob_JVec& ddthetalist,
									const Vector3d& g, const Vector6d& Ftip, const vector<SE3>& Mlist,
									const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist,double eef_mass) {
	    // the size of the lists
		int n = MOBILE_DOF_NUM;
	...
	Mob_JVec GravityForces(const Mob_JVec& thetalist, const Vector3d& g,
									const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist) {
	    int n = MOBILE_DOF_NUM;
		Mob_JVec dummylist = Mob_JVec::Zero();
		Vector6d dummyForce = Vector6d::Zero();
		Mob_JVec grav = InverseDynamics(thetalist, dummylist, dummylist, g,
                                                dummyForce, Mlist, Glist, Slist);
		return grav;
	}	

	Mob_JVec GravityForces(const Mob_JVec& thetalist, const Vector3d& g,
									const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist,double eef_mass) {
	    int n = MOBILE_DOF_NUM;
		Mob_JVec dummylist = Mob_JVec::Zero();
		Vector6d dummyForce = Vector6d::Zero();
		Mob_JVec grav = InverseDynamics(thetalist, dummylist, dummylist, g,
	...
	Mob_MassMat MassMatrix(const Mob_JVec& thetalist,
                                const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist) {
		int n = MOBILE_DOF_NUM;
		Mob_JVec dummylist = Mob_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		Vector6d dummyforce = Vector6d::Zero();
		Mob_MassMat M = Mob_MassMat::Zero();
		for (int i = 0; i < n; i++) {
			Mob_JVec ddthetalist = Mob_JVec::Zero();
			ddthetalist(i) = 1;
	...
	Mob_JVec VelQuadraticForces(const Mob_JVec& thetalist, const Mob_JVec& dthetalist,const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist) {
		int n = MOBILE_DOF_NUM;
		Mob_JVec dummylist = Mob_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		Vector6d dummyforce = Vector6d::Zero(6);
		Mob_JVec c = InverseDynamics(thetalist, dthetalist, dummylist,
                             dummyg, dummyforce, Mlist, Glist, Slist);
		return c;
	}	
	
		Mob_JVec VelQuadraticForces(const Mob_JVec& thetalist, const Mob_JVec& dthetalist,const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist,double eef_mass) {
		int n = MOBILE_DOF_NUM;
		Mob_JVec dummylist = Mob_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		Vector6d dummyforce = Vector6d::Zero(6);
	...
	Mob_JVec EndEffectorForces(const Mob_JVec& thetalist, const Vector6d& Ftip,const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist) {
		int n = MOBILE_DOF_NUM;
		Mob_JVec dummylist = Mob_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		Mob_JVec JTFtip = InverseDynamics(thetalist, dummylist, dummylist,
                             dummyg, Ftip, Mlist, Glist, Slist);
		return JTFtip;
	}
	
		Mob_JVec EndEffectorForces(const Mob_JVec& thetalist, const Vector6d& Ftip,const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const Mob_ScrewList& Slist,double eef_mass) {
		int n = MOBILE_DOF_NUM;
		Mob_JVec dummylist = Mob_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		Mob_JVec JTFtip = InverseDynamics(thetalist, dummylist, dummylist,
	...
	Mob_JVec ForwardDynamics(const Mob_JVec& thetalist, const Mob_JVec& dthetalist, const Mob_JVec& taulist,
		...
		Mob_JVec totalForce = taulist - VelQuadraticForces(thetalist, dthetalist, Mlist, Glist, Slist)
                 							 - GravityForces(thetalist, g, Mlist, Glist, Slist)
                                             - EndEffectorForces(thetalist, Ftip, Mlist, Glist, Slist);
		Mob_MassMat M = MassMatrix(thetalist, Mlist, Glist, Slist);
		// Use LDLT since M is positive definite
	    Eigen::LDLT<Mob_MassMat> ldlt(M);
		Eigen::VectorXd v = Eigen::Map<Eigen::VectorXd>(totalForce.data(), totalForce.size());
		Eigen::VectorXd x = ldlt.solve(v);
        // Mob_JVec ddthetalist = M.ldlt().solve(totalForce);
		Mob_JVec ddthetalist =  Eigen::Map<Mob_JVec>(x.data(), x.size());
		return ddthetalist;
	}	
	Mob_JVec ForwardDynamics(const Mob_JVec& thetalist, const Mob_JVec& dthetalist, const Mob_JVec& taulist,
									const Vector3d& g, const Vector6d& Ftip, const vector<SE3>& Mlist,
		...
		Mob_JVec totalForce = taulist - VelQuadraticForces(thetalist, dthetalist, Mlist, Glist, Slist,eef_mass)
                 							 - GravityForces(thetalist, g, Mlist, Glist, Slist,eef_mass)
                                             - EndEffectorForces(thetalist, Ftip, Mlist, Glist, Slist,eef_mass);
		Mob_MassMat M = MassMatrix(thetalist, Mlist, Glist, Slist,eef_mass);
		// Use LDLT since M is positive definite
	    Eigen::LDLT<Mob_MassMat> ldlt(M);
		Eigen::VectorXd v = Eigen::Map<Eigen::VectorXd>(totalForce.data(), totalForce.size());
		Eigen::VectorXd x = ldlt.solve(v);
	...
	void FKinBody(const SE3& M,const Mob_ScrewList& Blist, const Mob_JVec& q ,const Mob_JVec& dq, SE3 &T, Mob_Jacobian &Jb,Mob_Jacobian& dJb){
		Jb = Blist;
		dJb = Mob_Jacobian::Zero();
		T = SE3::Identity();
		SE3 T_ = SE3::Identity();
		Vector6d prev_dJidt=Blist.col(MOBILE_DOF_NUM-1)*dq(MOBILE_DOF_NUM-1);
		for(int i = MOBILE_DOF_NUM-2 ;i >= 0;i--){	
       ...
       Mob_Jacobian dJacobianBody(const SE3& M,const Mob_ScrewList& Blist, const Mob_JVec& q ,const Mob_JVec& dq){
		Mob_Jacobian Jb = Blist;
		Mob_Jacobian dJb = Mob_Jacobian::Zero();
		SE3  T = SE3::Identity();
		SE3 T_ = SE3::Identity();
		Vector6d prev_dJidt=Blist.col(MOBILE_DOF_NUM-1)*dq(MOBILE_DOF_NUM-1);
		for(int i = MOBILE_DOF_NUM-2 ;i >= 0;i--){	
...
void JointTrajectory(const Mob_JVec q0, const Mob_JVec qT, double Tf, double t , int method , Mob_JVec& q_des, Mob_JVec& q_dot_des, Mob_JVec& q_ddot_des) {
	...
	// RelMob_Jacobian RelJacobianSpace(const RelMob_ScrewList& Slist, const RelMob_JVec& thetaList) {
	// 	RelMob_Jacobian Js = Slist;	
	// 	SE3 T = SE3::Identity();
	...
	// 	return Js;
	// }
	// RelMob_Jacobian RelJacobianBody(const RelMob_ScrewList& Blist, const RelMob_JVec& thetaList) {
	// 	RelMob_Jacobian Jb = Blist;
	// 	SE3 T = SE3::Identity();
    ...
    //    RelMob_Jacobian ReldJacobianBody(const SE3& M,const RelMob_ScrewList& Blist, const RelMob_JVec& q ,const RelMob_JVec& dq){
	// 	RelMob_Jacobian Jb = Blist;
	// 	RelMob_Jacobian dJb = RelMob_Jacobian::Zero();
	// 	SE3  T = SE3::Identity();
	// 	SE3 T_ = SE3::Identity();
	...
	// }		
	// bool RelIKinSpace(const RelMob_ScrewList& Slist, const SE3& M, const SE3& T,
	// 	RelMob_JVec& thetalist, double eomg, double ev) {
	// 	int i = 0;
	// 	int maxiterations = 20;
	...
	// bool RelIKinBody(const RelMob_ScrewList& Blist, const SE3& M, const SE3& T,
	// 	RelMob_JVec& thetalist, double eomg, double ev) {
	// 	int i = 0;
	// 	int maxiterations = 20;
