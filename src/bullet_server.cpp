...
void *bullet_run(void *arg)
{
    ...
    int nomId = b3sim->loadURDF("/home/robot/robot_ws/RTECAT_Mobile/description/hyumob.urdf");
    int actId = b3sim->loadURDF("/home/robot/robot_ws/RTECAT_Mobile/description/hyumob.urdf");

    b3sim->setRealTimeSimulation(false);
    b3robot_nom = new Bullet_Mobile(b3sim,nomId);	
    b3robot_act = new Bullet_Mobile(b3sim,actId);	
	...
	while(true) 
    {
        ...
        Eigen::Map<Mob_JVec> q_sim(bullet_nom->position, MOBILE_DOF_NUM);

        ret = read(sockfd_act, bullet_act, BUFLEN_BULLET);
        Eigen::Map<Mob_JVec> q_act(bullet_act->position, MOBILE_DOF_NUM);	

        b3robot_nom->reset_q(q_sim);		
        b3robot_act->reset_q(q_act);
        std::cout<<q_sim(0)<<", "<<q_sim(1)<<", "<<q_sim(2)<<std::endl;
        std::cout<<q_act(0)<<", "<<q_act(1)<<", "<<q_act(2)<<std::endl;
